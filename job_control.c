...
void get_command(char *inputBuffer, int size, char *args[], int *background, int* respawneable)
{
    ...
    *respawneable=0;
    ...
    for (i = 0; i < length; i++) { 
        ...
        default :             /* some other character */
            ...
            else if(inputBuffer[i] == '+'){ //Respawneable indicator
                *respawneable=1;
                if(start!=-1){
                    args[ct] = &inputBuffer[start];     
                    ct++;
...
// -----------------------------------------------------------------------
/* inserta elemento respawneable en la cabeza de la lista */
void add_respawneable_job (job * list, job * item, char **args)
{
    job * aux = list->next;
    item->args = (char**) malloc(sizeof(args));
    list->next = item;
    item->next = aux;
